...
void main() {
    ...
    if(v.normal.w>.5 && v.normal.w<1.5) {
        float a=o.scene.x*.17+.23;
        mat2 r=mat2(cos(a),sin(a),-sin(a),cos(a));
        p.xy=r*p.xy; n.xy=r*n.xy;
    }
    if(material==5) {
        p.z=-9.9+(p.z+9.9)*step(.01,o.flash);
    }
    if(v.normal.w<2.5) p.z+=o.scene.z;
    if(v.normal.w>1.5 && v.normal.w<2.5) p.y+=.15*sin(o.scene.x*.8+p.x);
    world=p; normal=n; tint=v.color; tint.a=step(2.5,v.normal.w);
    if(v.normal.w<2.5) tint.rgb*=smoothstep(4.5,5.1,o.scene.x);
    gl_Position=o.padding<-.5?omegaShadow(p):o.vp*vec4(p,1);
    if(v.normal.w<2.5 && o.scene.x<4.5) gl_Position=vec4(2,2,2,1);
}
