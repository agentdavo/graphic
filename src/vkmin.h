...
enum { VKMIN_MAX_TIMESTAMPS = 32, VKMIN_PUSH_BYTES = 128, VKMIN_MAX_FRAME_LIST = 64 };
/* The GPU-side limits live in vkmin_gpu.h, which glslang compiles too:
 * VKMIN_MAX_TEXTURES and the VKMIN_SAMPLER_* presets. Tuning either side of
 * that split means editing the other header, not this one. */

typedef struct vkmin_ctx vkmin_ctx;
typedef struct cvar_state cvar_state;
...
/* ------------------------------------------------------------ the flags -- */
/* One table, read by the parser and by --flags. A list written twice drifts:
 * this one carried a flag that takes milliseconds documented as taking a file,
 * and a flag the headless path never consults documented without that caveat.
 * X(id, text, words, help): the C identifier, the spelling after the dashes,
 * how many argv words follow it, and one line of help. A listed flag missing
 * its words is an error; anything not listed belongs to the program, as do
 * cvar assignments in name=value or +name value form. */
#define VKMIN_FLAG_LIST(X)                                                                          \
    X(headless,         "headless",         0, "render with no window")                             \
    X(frame,            "frame",            1, "N: this frame alone, headless")                     \
    X(frames,           "frames",           1, "A,B,C: these frames, headless; at most VKMIN_MAX_FRAME_LIST") \
    X(out,              "out",              1, "PATH: write the rendered frame here")               \
    X(out_dir,          "out-dir",          1, "DIR: write every rendered frame here")              \
    X(exit_after,       "exit-after",       1, "N: end the loop after N frames; windowed only, the "\
                                               "headless loop ends when the frame list runs out")   \
    X(size,             "size",             2, "W H: render size, the same as r_width and r_height")\
    X(device,           "device",           1, "N: physical device index")                          \
    X(path_legacy,      "path=legacy",      0, "force staging uploads and shader modules")          \
    X(path_modern,      "path=modern",      0, "force host image copy and inline SPIR-V")           \
    X(sync_naive,       "sync-naive",       0, "one frame in flight, wait idle per submit")         \
    X(no_readback,      "no-readback",      0, "legacy path: skip the per-frame capture copy")      \
    X(verbose,          "verbose",          0, "print the device, its features and the chosen path")\
    ...
    X(events,           "events",           1, "FILE: write the inspection event log")              \
    X(inspect_dir,      "inspect-dir",      1, "DIR: write inspection images here")                 \
    X(stop_after_event, "stop-after-event", 1, "N: stop once inspection event N is reached")        \
    X(metrics,          "metrics",          1, "FILE: write frame and memory totals as JSON at shutdown") \
    X(budget,           "budget",           1, "MS: fail at shutdown when the p99 frame time exceeds this")

typedef struct {
    int argc; char **argv;        /* the command line; see VKMIN_FLAG_LIST above */
    ...
    const cvar_state *config;     /* optional initial state, copied at init; NULL = fresh defaults */
    /* Reserved once at init and never grown, so these are the program's whole
     * device budget. Cvars r_arena_mb, r_image_arena_mb and r_ring_mb override
     * them from the command line; stats.device_cap reports what was taken and
     * arena_high_water what was ever used. */
    size_t device_arena_bytes;    /* buffers; 0 = 256 MB */
    size_t image_arena_bytes;     /* images; 0 = device_arena_bytes */
    size_t host_ring_bytes;       /* 0 = 64 MB. Caps one upload chunk. */
    bool history;                /* render preceding frames for --frame/--frames; set before init */
    FILE *journal;               /* borrowed min_jrnl.h stream, shared with audio; 0 = disabled */
} vkmin_desc;
...
/* Borrowed until shutdown. Edits take effect at the next frame begin. */
cvar_state *vkmin_config(vkmin_ctx *);                                       // writes ctx
 ...
 * descriptors are reclaimed after prior submissions finish. Never submit a
 * freed GPU address or texture index again. Allocation may wait for retirement
