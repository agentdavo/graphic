...
void main() {
    ...
    // One texel step, whatever r_omega_shadow made the map. The literal 2048
    // that used to be here silently became a sub-texel blur at any other size.
    vec2 shadowTexel=1./vec2(textureSize(TEX(o.texture_id),0));
    for(int y=-1;y<=1;y++) for(int x=-1;x<=1;x++)
        visibility+=step(sc.z-bias,texture(TEX(o.texture_id),st+vec2(x,y)*shadowTexel).r)/9.;
    ...
    if(structure<.5) {
        for(int sx=-1;sx<=1;sx+=2) for(int sy=-1;sy<=1;sy+=2) {
            vec3 engine=vec3(float(sx)*OMEGA_ENGINE_X,float(sy)*OMEGA_ENGINE_Y,16.);
            engine=opponent?omegaOpponentRotate(engine)+omegaFormation(ship):engine+vec3(0,0,F.scene.z);
            vec3 delta=engine-world;
