...
static settings read_settings(const cvar_state *config) {
    settings s = {
        .shadows = cvar_get_bool(config, CV_r_shadows),
        .normal_maps = cvar_get_bool(config, CV_r_normal_maps),
        .clustered = cvar_get_bool(config, CV_r_clustered),
        .prepass = cvar_get_bool(config, CV_r_prepass),
        .gpu_cull = cvar_get_bool(config, CV_r_gpu_cull),
        .transparent = cvar_get_bool(config, CV_r_transparent),
        .overlay = cvar_get_bool(config, CV_r_overlay),
        .freeze = cvar_get_bool(config, CV_r_freeze_cull),
        .compact = cvar_get_bool(config, CV_r_cull_compact),
        ...
        .debug = cvar_get_int(config, CV_r_debug),
        .tonemap = cvar_get_int(config, CV_r_tonemap),
        .shadow_bias = cvar_get(config, CV_r_shadow_bias),
        .normal_bias = cvar_get(config, CV_r_normal_bias),
        .cascade_lambda = cvar_get(config, CV_r_cascade_lambda),
        .exposure = cvar_get(config, CV_r_exposure),
        .shadow_distance = cvar_get(config, CV_r_shadow_distance),
        .max_lights = (uint32_t)cvar_get_int(config, CV_r_max_lights),
    };
...
void vkr_frame(vkr *r, const vkr_frame_desc *f) {
    ...
    vkmin_ctx *gpu = r->gpu;
    const cvar_state *config = vkmin_frame_config(gpu);
    settings s = read_settings(config);
    ...
    const bool use_taa = outside && cvar_get_bool(config, CV_taa);
    const float bloom_strength = outside ? cvar_get(config, CV_bloom) : 0;
