...
int main(int argc, char **argv) {
    ...
    const vkmin_pipeline fill = vkmin_make_pipeline(c, &(vkmin_pipeline_desc){.cs = VKMIN_BYTES(smoke_comp_spv), .push_size = sizeof(Push), .label = "smoke.fill"});
    const vkmin_pipeline draw = vkmin_make_pipeline(
        c, &(vkmin_pipeline_desc){.vs = VKMIN_BYTES(smoke_vert_spv), .push_size = sizeof(Push),
                              .fs = VKMIN_BYTES(smoke_frag_spv),
    ...
    while (vkmin_running(c)) {
        ...
        vkmin_draw(c, draw, &draw_push, 3, 1);
        vkmin_pass_end(c);
        vkmin_timestamp(c, 2);
        vkmin_frame_end(c);
    }
