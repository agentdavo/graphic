...
- Formatting and language conventions were written down in `docs/style.md` and the mechanical
  ...
  known-good shapes before it is trusted, because a check that quietly becomes a no-op is
  worse than no check, and this one did exactly that once, reporting success while
  twenty-seven violations sat in the tree.
...
## 7. vkmin is not a validation layer
...
rule the specification states. When a caller does something illegal, the fix is to run with
validation on and read what it says, not to grow a check that says it worse.

What does belong here is the part no layer can know:

- Preconditions that are vkmin's own invention — arena bounds, host-ring offsets, journal
  record framing, the C/GLSL transport contract. Nobody else checks those, so we must.
- Asserts for programmer error. Littering asserts while tracking a failure still stands; an
  assert is a debugging tool, not a shipped policing layer.
- One check at the boundary, not defensive re-checks at every level below it.

Wrapper thinness is a feature with the same standing as correctness. Every convenience that
hides a Vulkan concept is a concept the reader now has to learn twice, and every guard rail
is code that has to be right forever. When a proposed change starts to look like error
reporting for the underlying API, assume it is a duplicate until shown otherwise.

## 8. How Claude should work here specifically
...
## 9. Toolchain on this Windows machine

The compilers live in MSYS2 at `C:\msys64`. Nothing is on the default PATH, so
put `C:/msys64/ucrt64/bin` and `C:/msys64/usr/bin` on it before building; the
`clang64` toolchain is present too. `cmake`, `ninja` and `mingw32-make` all come
from `ucrt64`, and `find_package(Vulkan)` reads `VULKAN_SDK` for both the
loader and `glslangValidator`.

- The build is CMake in the project root; the three Makefiles it replaced are
  gone from the tree but remain in git history.
...
- Under Windows PowerShell 5.1 a native program that writes to stderr while
  ...
  its device line to stderr and succeeds.
