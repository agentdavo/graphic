...
static inline gk_options gk_parse(int argc, char **argv, const char *help_text) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (!strcmp(a, "--headless") || !strcmp(a, "--frame") || !strcmp(a, "--frames") || !strcmp(a, "--replay")) o.headless = true;
            ...
            if (!cvar_parse_assignment(&o.config, joined)) exit(2);
        }
        else if (a[0] != '-' && strchr(a, '=')) { if (!cvar_parse_assignment(&o.config, a)) exit(2); }
    }
    if (o.profile) {
        ...
        for (size_t k = 0; k < sizeof profile / sizeof profile[0]; ++k) {
            if (!cvar_was_set(&o.config, profile[k].id)) cvar_set(&o.config, profile[k].id, profile[k].v);
        }
    }
    return o;
}
